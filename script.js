import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
        ...
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'uno-thai-web-game';
        ...
        let pendingWildCard = null; // Store card data temporarily if wild played
        ...
        async function initAuth() {
            try {
                ...
                showToast("เกิดข้อผิดพลาดในการยืนยันตัวตน", "error");
            }
        }
        ...
        document.getElementById('btn-create-room').addEventListener('click', async () => {
            ...
            try {
                ...
                showModal("ข้อผิดพลาด", "ไม่สามารถสร้างห้องได้");
            }
        });
        ...
        async function joinRoom(roomId, isRejoin = false) {
            ...
            try {
                ...
                // If not rejoining and not already in room, add to players
        ...
        window.leaveRoom = async function() {
            ...
            try {
                ...
                if (updatedPlayers.length === 0 || (roomData.hostId === currentUser.uid && updatedPlayers.filter(p => !p.isBot).length === 0)) {
                    // Everyone left or only bots left after host leaves -> delete room conceptually (clear data or set status)
                    // We just let it be or set status to ended, for safety let's just leave it, no cloud func to delete.
                } else {
        ...
        function updateUIBasedOnState() {
            ...
            if (roomData.status === 'waiting') {
                ...
                roomData.players.forEach(p => {
                    ...
                    list.innerHTML += `
                        <div class="flex items-center gap-3 bg-gray-700/50 p-2 rounded-lg border ${isMe ? 'border-blue-500' : 'border-transparent'}">
                            ...
                            ${isHost && !isMe && !p.isBot ? `<button onclick="kickPlayer('${p.id}')" class="ml-auto text-red-400 hover:text-red-300 px-2 py-1"><i class="fas fa-times"></i></button>` : ''}
                            ${isHost && p.isBot ? `<button onclick="kickPlayer('${p.id}')" class="ml-auto text-red-400 hover:text-red-300 px-2 py-1"><i class="fas fa-times"></i></button>` : ''}
                        </div>
                    `;
                });
                ...
                showModal("เกมจบแล้ว!", `ผู้ชนะคือ: ${winnerName}`, () => {
                    // Host can return to lobby or waiting room
                    if (isHost) {
                       resetToWaiting();
                    } else {
                       // Just wait for host to change state
                       switchScreen('waiting');
                    }
                });
            }
        ...
        document.getElementById('btn-start-game').addEventListener('click', async () => {
            ...
            // First card for discard pile (no wild/special for first card to keep it simple, keep drawing until normal)
            let topCard = null;
            let drawIndex = 0;
            ...
            if(!topCard) { topCard = deck.splice(0,1)[0]; } // fallback
            ...
            const initialGameState = {
                ...
                currentColor: topCard.c !== 'wild' ? topCard.c : 'red', // fallback if first card is wild somehow
                drawStack: 0,
                winner: null,
                unoCallers: [],
            ...
            // Reset players array to update card counts
        ...
        function generateDeck() {
            ...
            // Wild cards (4 wild, 4 draw4)
            for (let i = 0; i < 4; i++) {
                deck.push({ id: `c_${idCounter++}`, c: 'wild', v: 'wild' });
                deck.push({ id: `c_${idCounter++}`, c: 'wild', v: 'draw4' });
            }

            // Special Action Cards (2 Swap, 2 Rotate)
            for (let i = 0; i < 2; i++) {
                deck.push({ id: `c_${idCounter++}`, c: 'wild', v: 'swap' });
                deck.push({ id: `c_${idCounter++}`, c: 'wild', v: 'rotate' });
            }
        ...
        function renderGameUI() {
            ...
            discardPile.innerHTML = getCardMarkup(topCard, false); // Not clickable
            ...
            // Current color indicator (important when wild is played)
            ...
            if (isMyTurn) {
                ...
                // Play simple alert sound (browser safe if user interacted)
            } else {
            ...
            logList.innerHTML = '';
            if (roomData.logs) {
                const displayLogs = roomData.logs.slice(-10).reverse();
        ...
        function renderOpponents() {
            ...
            // Find my index to order opponents relatively
            let myIdx = roomData.players.findIndex(p => p.id === currentUser.uid);
            if(myIdx === -1) myIdx = 0; // fallback if observer
        ...
        function isCardPlayable(card, topCard, currentColor, drawStack) {
            // If there's an active +2 or +4 stack, you MUST play a matching + card, OR draw
            if (drawStack > 0) {
            ...
            // Normal rules
            if (card.c === 'wild') return true; // Wilds always playable
            if (card.c === currentColor) return true; // Matches color
            if (card.v === topCard.v) return true; // Matches number/symbol
        ...
        window.drawCard = async function() {
            ...
            // Call logic with null card means draw
        ...
        window.callUno = async function() {
            ...
            const myHand = roomData.hands[currentUser.uid];
            // Can only call UNO if 2 cards before playing (leaving 1) or 1 card if forgot
            if (myHand.length <= 2) {
                ...
                if (!callers.includes(currentUser.uid)) {
                    ...
                    document.getElementById('btn-uno').classList.remove('animate-bounce');
                    document.getElementById('btn-uno').classList.add('opacity-50', 'cursor-not-allowed');
                }
            }
        };

        async function executeTurn(playedCard, chosenColor) {
            let state = JSON.parse(JSON.stringify(roomData)); // Deep copy to mutate locally then upload
            ...
            if (playedCard) {
                ...
                if (playedCard.v === 'skip') {
                    ...
                    state.players.forEach(p => {
                        p.cardCount = state.hands[p.id].length;
                    });
                    
                    logs.push(`🌪️ ${currentPlayer.name} ใช้การ์ดหมุนไพ่ทั้งวง!`);
                }

                // Check Win หลังจบเอฟเฟกต์ของการ์ด (หากแลกไพ่ทำให้คนอื่นมือหมด คนนั้นจะชนะทันที)
                ...
                if (state.drawStack > 0) {
                    ...
                    // If playing strictly, drawn card can be played immediately if valid. 
                    // To keep UI simple for mobile web: Drawing ends turn immediately.
                    state.turnIndex = getNextIndex(state.turnIndex, state.direction, state.players.length);
                }
            }

            // Clean up unoCallers if their turn ended and they have != 1 card
        ...
        function drawCardsForPlayer(state, playerId, count) {
            for (let i = 0; i < count; i++) {
                if (state.deck.length === 0) {
                    ...
                    if (state.deck.length === 0) break; // still empty? prevent infinite loop
                }
        ...
        async function commitState(newState) {
            ...
            try {
                ...
                showToast("เครือข่ายขัดข้อง กรุณาลองใหม่", "error");
            }
        }
        ...
        function checkBotTurn() {
            ...
            // Only the Host handles bot turns to prevent duplicate writes
        ...
        async function executeBotTurn(botPlayer) {
            // Re-fetch latest from roomData in case it changed
            ...
            if (playableCards.length > 0) {
                // Strategy: Prefer matching color, then matching value, then wild. 
                // Also, if bot has 2 cards, call UNO!
                
                // Simulate calling UNO randomly before playing
                if (hand.length === 2) {
                    let callers = roomData.unoCallers || [];
                ...
                if (!cardToPlay) cardToPlay = playableCards[0]; // Fallback to wild
